...
contract ERC223 {
  ...
  function Vikitransfer(address to, uint value, bytes data) public returns (bool ok);
  ...
  */
    ...
    returns (uint256)
  {
    ...
    function transfer(address _to, uint _value)
        public
        returns (bool) {
        return Vikitransfer(_to, _value, "");
    }

    function Vikitransfer(address _to, uint _value, bytes _data)
        public
        returns (bool) {
        if (_value > 0) {
...
contract BetaToken is MintableToken, BurnableToken {
  ...
  * @param _value number of tokens to be transfered.
   */
    ...
    function Vikitransfer(address _to, uint _value, bytes _data) onlyIfCanMove public returns (bool) {
        return super.Vikitransfer(_to, _value, _data);
    }
...
contract Beta_Wallet_for_HoneyGramm {
    ...
    modifier OnlyPermitting() {

        require((msg.sender == _owner) || (msg.sender == admin) || (confirmations[transactionId][msg.sender] == true));
        _;
    }
...
//ezt külön kell venni vagy ez mindig tokent küld
    ...
    function tesztelAPA(address _who) public view returns (bool)
         { 
        return ((msg.sender == _owner) || (msg.sender == admin) || (confirmations[transactionId][msg.sender] == true));
    }
}